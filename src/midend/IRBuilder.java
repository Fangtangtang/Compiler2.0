...
public class IRBuilder implements ASTVisitor<Entity> {
    ...
    //重命名后的name -> Storage
    public HashMap<String, Storage> rename2mem = new HashMap<>();
    ...
    public Entity visit(FuncDefStmtNode node) {
        ...
        if (currentScope instanceof GlobalScope) {
            currentFunction = new Function(entity.type, node.name, currentInitBlock);
        }
        //类成员函数
        else {
            ...
            currentFunction = new Function(
                    entity.type,
                    classScope.classType.name + "." + node.name,
                    currentInitBlock
            );
    ...
    public Entity visit(VarDefUnitNode node) {
        ...
        if (currentScope instanceof GlobalScope) {
            ...
            //将rename -> mem映射存入map
            rename2mem.put(name, stmt.result.storage);
            //非字面量初始化
            //在全局的初始化函数中加入赋值语句
